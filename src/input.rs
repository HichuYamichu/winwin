...
pub enum Key {
    ...
    VK_PAUSE = 0x13,
    VK_CAPITAL = 0x14,
    VK_KANA_VK_HANGUL = 0x15,
    // VK_KANA = 0x15,
    // VK_HANGUL = 0x15,
    VK_IME_ON = 0x16,
    VK_JUNJA = 0x17,
    VK_FINAL = 0x18,
    VK_HANJA_VK_KANJI = 0x19,
    // VK_HANJA = 0x19,
    // VK_KANJI = 0x19,
    VK_IME_OFF = 0x1A,
    VK_ESCAPE = 0x1B,
...
pub struct Input {
    keys: [u32; 8],
    intercept_tx: Sender<KeyboardOp>,
}
...
impl Input {
    pub fn new(keys: [u32; 8], tx: Sender<KeyboardOp>) -> Self {
        Input {
            keys: keys,
            intercept_tx: tx,
        }
    ...
    pub fn pressed(&self, keys: &[Key]) -> bool {
        let all_pressed = self.pressed_no_capture(keys);
        if all_pressed {
    ...
    pub fn pressed_no_capture(&self, keys: &[Key]) -> bool {
        let pressed_count: u32 = self.keys.iter().map(|chunk| chunk.count_ones()).sum();
        if pressed_count as usize != keys.len() {
            return false;
        }

        keys.iter().all(|key| {
            let vk_code = *key as u8;
